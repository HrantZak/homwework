...
struct TodayView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ZStack(alignment: .bottomLeading) {
                        ...
                        Circle().fill(.white.opacity(0.13)).frame(width: 180).offset(x: 220, y: -55)
                    ...
                    }.frame(height: 180).clipShape(RoundedRectangle(cornerRadius: 30, style: .continuous))
            ...
            }.background(AppTheme.background).navigationTitle("Успевай")
        }
    }
}
